...
def concept_explanation():
    st.subheader("generate video using AI")
    topic = st.text_input("Enter the topic:")

    with st.sidebar:
        st.markdown("## Style")
        style_selected = st.radio("", ["Image", "Video"])
...
def study_mode():
    ...
    #     question_from_pdf()
    if selected_page == "Question from Image":
        question_from_image()
    elif selected_page == "video gen":
        concept_explanation()
...
mode = st.sidebar.radio("Choose a mode:", ["vid"])
